...
spinChangeIntake :: (RunClient m, MonadTime m, MonadGen m, MonadThrow m, MonadLogger m, MonadCatch m, MonadBase IO m, MonadRandom m) => SpinChangeConfig -> AppianT m (Maybe Text)
spinChangeIntake conf = do
  v <- myLandingPageAction "SPIN Change"

  spinChangeIntake' conf v

spinChangeIntake' :: (RunClient m, MonadTime m, MonadGen m, MonadThrow m, MonadLogger m, MonadCatch m, MonadBase IO m, MonadRandom m) => SpinChangeConfig -> Value -> AppianT m (Maybe Text)
...
printFRNs :: (RunClient m, MonadTime m, MonadThrow m, MonadLogger m, MonadCatch m, MonadBase IO m, MonadRandom m) => Value -> GridField GridFieldCell -> AppianT m (Value, Value)
printFRNs val gf = do
  v <- foldGridField (printFRN val) "FRN" val gf
  return (v, v)

printFRN :: (RunClient m, MonadTime m, MonadThrow m, MonadLogger m, MonadCatch m, MonadBase IO m, MonadRandom m) => Value -> Value -> GridFieldCell -> AppianT m Value
...
selectNewSPIN :: (RunClient m, MonadTime m, MonadThrow m, MonadLogger m, MonadCatch m, MonadBase IO m, MonadRandom m) => Int -> Value -> AppianT m Value
...
selectOldSPIN :: (RunClient m, MonadTime m, MonadThrow m, MonadLogger m, MonadCatch m, MonadBase IO m, MonadRandom m) => Value -> AppianT m Value
selectOldSPIN v = do
