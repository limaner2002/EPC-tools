...
type LandingPageActionEx = "suite" :> "rest" :> "a" :> "model" :> "latest" :> PathPiece ProcessModelId :> EmptyAppianTV :> Header "X-Appian-Features" Text :> AppianCsrfToken :> Post '[AppianTVUI] Value
...
landingPageActionEx pid = Appian (landingPageActionEx_ pid (Just "ceebc"))
...
sendPicker f pathPiece label uname v = handleMissing label =<< (sequence $ f pathPiece <$> up)
  where
    up = join $ mkUpdate <$> (_Just . pwValue .~ uname $ apd) <*> pure v
    apd = v ^? getPickerWidget label

pickerUpdate :: Text -> AppianPickerData -> Value -> Maybe (UiConfig (SaveRequestList Update))
pickerUpdate label uname v = up
  where
    up = join $ mkUpdate <$> (_Just . pwValue .~ uname $ apd) <*> pure v
    apd = v ^? getPickerWidget label
